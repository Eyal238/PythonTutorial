...
# 4.  כתוב פונקציה המקבלת 2 מספרים, הפונקציה תחזיר את המכפלה שלהם אך ללא שימוש בפעולת הכפל.
def f4(x,y):
    z=0
    while x>0:
        z=z+y
...
# 5.  כתוב פונקציה המקבלת מספר ובודקת האם הוא חלק מסדרת פיבונאצ'י. הפונקציה תחזיר חיובי או שלילי.
...
def f5(n):
   ...
   if i==n:
       print("False")
 #  6.  כתוב פונקציה המקבלת x - המהווה מחלק, ובנוסף y - המהווה מחולק, הפונקציה תדפיס את המנה ללא שימוש בפעולת החילוק.
def f2(x,y):
    i=0
    count=0
    while(i<x):
